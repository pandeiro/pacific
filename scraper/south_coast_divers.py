...
class SouthCoastDiversScraper(BaseScraper):
    ...
    async def scrape(self) -> List[Any]:
        ...
        if not dive_report_text:
            self.logger.warning(f"No dive report found on page")
            return []

        self.logger.info(f"Extracted dive report ({len(dive_report_text)} chars)")

        # Parse the date from the dive report text (e.g., "03/20/2026")
        report_date = self._parse_report_date(dive_report_text)
        if report_date:
            timestamp = report_date
            self.logger.info(f"Parsed report date: {timestamp.strftime('%Y-%m-%d')}")
        else:
            timestamp = datetime.now(timezone.utc)
            self.logger.warning(f"Could not parse report date, using current time")
        ...
        async with get_db_session() as session:
            ...
            if is_duplicate:
                self.logger.info(
                    f"Duplicate report found (within last 96 hours), skipping"
                )
                return []
        ...
        self.logger.info(f"Archived dive report text")
        ...
        try:
            async with LLMClient() as llm_client:
                ...
                self.logger.info(f"LLM extraction: {conditions_data}")
                ...
                if not valid_format:
                    self.logger.info(
                        f"LLM returned invalid format, using fallback extraction"
                    )
                    conditions_data = fallback_extraction(dive_report_text)
                    self.logger.info(f"Fallback extraction: {conditions_data}")

                # Process the extraction results
                if conditions_data is None or not isinstance(conditions_data, dict):
                    self.logger.info(
                        f"ERROR: LLM returned invalid response type: {type(conditions_data)}"
                    )
                    self.logger.info(
                        f"Dive report will be archived without extraction"
                    )
                elif (
                    ...
                    self.logger.info(
                        f"WARNING: LLM returned empty extraction (no visibility/swell found)"
                    )
                    self.logger.info(f"Raw response: {conditions_data}")
                else:
            ...
            self.logger.error(f"ERROR: LLM extraction failed: {e}")
            self.logger.info(f"Dive report will be archived without extraction")
            if records:
                dive_report_id = records[0].get("id") if records else None
                self.logger.info(f"Dive report ID: {dive_report_id}")
        ...
        if vis_range:
            ...
            if vis_min is not None:
                ...
                self.logger.info(f"Extracted visibility: {vis_min}-{vis_max} ft")
        ...
        if swell_range:
            ...
            if swell_min is not None:
                ...
                self.logger.info(f"Extracted swell: {swell_min}-{swell_max} ft")

        self.logger.info(f"Persisting {len(records)} records to database...")
        async with get_db_session() as session:
            await insert_conditions(session, records)

        self.logger.info(f"Successfully persisted {len(records)} records")
        return records
    ...
    def _extract_dive_report(self, html_content: str) -> Optional[str]:
        ...
        if not target_element:
            self.logger.warning(f"Could not find target text '{target_text}'")
            return None

        table = target_element.find_next("table")

        if not table:
            self.logger.warning(f"No table found after target element")
            return None
